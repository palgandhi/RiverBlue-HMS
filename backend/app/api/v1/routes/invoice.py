...
async def get_invoice_html(
    ...
    for item in room_items:
        item_rows_html += f"""<tr>
            <td>{sno}</td><td>{item.description}</td><td>9963</td>
    ...
    for item in extra_items:
        ...
        badge = f'<span class="badge {item.item_type}">{item.item_type}</span>'
        item_rows_html += f"""<tr class="{"discount-row" if item.item_type == "discount" else ""}">
            <td>{sno}</td><td>{item.description} {badge}</td><td>-</td>
            <td class="right">{item.quantity}</td>
            <td class="right">{sign}{fmt(abs(item.unit_price))}</td>
            <td class="right {"green" if item.item_type == "discount" else ""}">{sign}{fmt(abs(item.total_price))}</td>
        </tr>"""
        sno += 1
    ...
    for p in payments:
        ...
        payment_rows_html += f"""<tr>
            <td>{paid_dt.strftime("%d %b %Y %H:%M")}</td>
    ...
    payments_section = ""
    if payments:
        payments_section = f"""<h3 style="margin:20px 0 8px;color:#1F4E79;font-size:13px;">Payment History</h3>
        <table>
          <thead><tr>
    ...
    if inv.discount_amount:
        discount_row = f"""<div class="totals-row discount">
            <span>Discount Applied</span><span>-{fmt(inv.discount_amount)}</span>
        </div>"""

    balance_row = ""
    if balance_due > 0:
        balance_row = f'<div class="totals-row red"><span>Balance Due</span><span>{fmt(balance_due)}</span></div>'
    else:
        balance_row = '<div class="totals-row green"><span>Balance Due</span><span>NIL</span></div>'

    gstin_badge = ""
    if hotel.gstin:
        gstin_badge = f'<div class="gstin-badge">GSTIN: {hotel.gstin}</div>'

    addr2 = f", {hotel.address_line2}" if hotel.address_line2 else ""
    phone_email = ""
    if hotel.phone:
        phone_email += f"Tel: {hotel.phone}"
    if hotel.email:
        phone_email += f" | {hotel.email}"

    guest_gstin_row = ""
    if guest.id_type:
        guest_gstin_row = f"<p>{guest.id_type}: {guest.id_number}</p>"

    guest_email_row = f"<p>{guest.email}</p>" if guest.email else ""
    children_str = f", {booking.num_children} children" if booking.num_children else ""
    website_str = hotel.website if hotel.website else ""
    gstin_footer = f"GSTIN: {hotel.gstin}" if hotel.gstin else ""
    cgst_row = f'<div class="totals-row"><span>CGST @ {cgst_rate}%</span><span>{fmt(cgst.total_price)}</span></div>' if cgst else ""
    sgst_row = f'<div class="totals-row"><span>SGST @ {sgst_rate}%</span><span>{fmt(sgst.total_price)}</span></div>' if sgst else ""
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .inv-number { font-size:13px; font-weight:bold; color:#C45A00; margin-top:4px; }
  ...
  .discount-row td { background:#f0fdf4 !important; }
  ...
  .totals-row.bold { font-weight:bold; font-size:14px; color:#1F4E79; border-top:2px solid #1F4E79; border-bottom:none; padding-top:8px; margin-top:4px; }
...
<div class="page">
  <div class="header">
    <div>
      ...
      <div class="hotel-details">""" + hotel.address_line1 + addr2 + """<br>""" + hotel.city + """ - """ + hotel.pincode + """, """ + hotel.state + """<br>""" + phone_email + """</div>
      """ + gstin_badge + """
    </div>
    <div class="invoice-title">
      ...
      <div style="margin-top:6px"><span class="status-badge status-""" + inv.status + """">""" + inv.status.replace("_"," ").title() + """</span></div>
    </div>
  </div>
  ...
  <div class="meta">
    ...
    <div class="meta-box">
      ...
      <p><strong>Check-in:</strong> """ + str(booking.check_in_date) + """</p>
      <p><strong>Check-out:</strong> """ + str(booking.check_out_date) + """</p>
      <p><strong>Guests:</strong> """ + str(booking.num_adults) + """ adults""" + children_str + """</p>
    </div>
  </div>
  ...
  <div class="totals">
    <div class="totals-box">
      ...
      """ + discount_row + """
      <div class="totals-row"><span>Taxable Amount</span><span>""" + fmt(taxable) + """</span></div>
      """ + cgst_row + """
      """ + sgst_row + """
      <div class="totals-row bold"><span>TOTAL</span><span>""" + fmt(inv.total_amount) + """</span></div>
      <div class="totals-row green"><span>Amount Paid</span><span>""" + fmt(total_paid) + """</span></div>
      """ + balance_row + """
    </div>
  </div>
  ...
  <div class="footer">
    <div>
      <strong>Computer-generated invoice. No signature required.</strong><br>
