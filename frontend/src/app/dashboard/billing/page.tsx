...
function BillingPageInner() {
  ...
  const [charge, setCharge] = useState({ description: "", item_type: "fb", quantity: "1", unit_price: "" });
  const [payment, setPayment] = useState({ amount: "", method: "cash", transaction_ref: "" });
  ...
  const handleFinalise = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.detail || "Failed to finalise invoice");
    }
  };
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <Card>
        <CardHeader className="pb-2 flex flex-row items-center justify-between">
          ...
          {isEditable && (
            <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => setChargeOpen(true)}>
        ...
        <DialogContent className="max-w-sm bg-card">
          ...
          <div className="space-y-3 mt-2">
            <div className="space-y-1.5">
              ...
              <Select value={charge.item_type} onValueChange={v => setCharge(c => ({ ...c, item_type: v }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {itemTypeOptions.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Description</Label>
              <Input placeholder="e.g. Club sandwich + fries" value={charge.description} onChange={e => setCharge(c => ({ ...c, description: e.target.value }))} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <Label className="text-xs">Quantity</Label>
                <Input type="number" min="1" value={charge.quantity} onChange={e => setCharge(c => ({ ...c, quantity: e.target.value }))} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">Unit Price (₹)</Label>
                <Input type="number" min="0" placeholder="0.00" value={charge.unit_price} onChange={e => setCharge(c => ({ ...c, unit_price: e.target.value }))} />
              </div>
            </div>
            {charge.unit_price && charge.quantity && (
              <div className="bg-muted/50 rounded p-2 text-sm text-center">
                Total: <span className="font-semibold">₹{(parseFloat(charge.unit_price || "0") * parseInt(charge.quantity || "1")).toLocaleString("en-IN")}</span>
              </div>
            )}
        ...
        <DialogContent className="max-w-sm bg-card">
          ...
          <div className="space-y-3 mt-2">
            <div className="bg-amber-50 border border-amber-200 rounded p-3 text-sm">
              ...
              <span className="font-bold text-amber-900">₹{(balance_due / 100).toLocaleString("en-IN")}</span>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input type="number" min="0" placeholder="0.00" value={payment.amount} onChange={e => setPayment(p => ({ ...p, amount: e.target.value }))} />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs">Transaction Reference (optional)</Label>
              <Input placeholder="UPI ref, card last 4 digits..." value={payment.transaction_ref} onChange={e => setPayment(p => ({ ...p, transaction_ref: e.target.value }))} />
            </div>
            <div className="flex gap-2 pt-1">
              <Button className="flex-1 bg-green-600 hover:bg-green-700" onClick={handlePayment} disabled={submitting}>
